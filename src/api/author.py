...
async def get_all_authors(db: DBDep, pagination: PaginationDep):
    ...
    authors = authors[pagination.per_page * (pagination.page - 1) :][
        : pagination.per_page
    ]
...
async def edit_author(
    ...
    author_data: AuthorPatch = Body(
        openapi_examples={
            "1": {
                "summary": "author_1",
                "value": {"last_name": "Грибоедов", "birth_date": "1795-01-15"},
            },
            "2": {
                "summary": "author_2",
                "value": {
    ...
    try:
        edited_author = await AuthorService(db).edit_author(
            id=id, author_data=author_data
        )
